...
import { RotateCcw, Award, BookOpen, Video, Sparkles, ExternalLink } from 'lucide-react';
...
export const BlackboardTracing: React.FC<BlackboardTracingProps> = ({ letter }) => {
  ...
  // Initialize the blackboard canvas and clear drawings
  const initCanvas = () => {
  ...
  // Re-run setup when letter or letter case toggles
  useEffect(() => {
    initCanvas();
    const handleResize = () => {
      initCanvas();
  ...
  const saveProgressAndShowWin = () => {
    ...
    // Save progress to localStorage for Parent Dashboard
    try {
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 w-full items-start">
      ...
      <div className="xl:col-span-7 flex flex-col gap-3">
        <div ref={containerRef} id="blackboard-tracing-container" className="relative w-full aspect-[4/3] rounded-3xl overflow-hidden border-8 border-amber-950 bg-[#163a2c] p-1 flex flex-col shadow-xl">
          ...
          <div className="relative flex-1 cursor-crosshair overflow-hidden">
            ...
            <div className="absolute bottom-4 left-4 text-xs font-mono text-[#a2d4be]/80 pointer-events-none z-10">
              ✍️ Bé hãy viết chữ "{isUppercase ? letter.uppercase : letter.lowercase}" thật đẹp lên bảng nhé! Viết xong bé hãy bấm "Nộp bài ✨" nha!
            </div>
                ...
                >
                  ...
                  >
                    ...
                    <p className="text-emerald-100 text-sm mb-5 leading-relaxed">
                      Bé đã tự mình hoàn thành luyện viết chữ <strong className="text-yellow-300 text-lg">{isUppercase ? letter.uppercase : letter.lowercase}</strong> rồi! Thỏ Trí Tuệ khen bé ngoan nha!
                    </p>
                    <div className="flex gap-3">
                      ...
                      <button
                        ...
                        className="px-5 py-2 bg-yellow-400 hover:bg-yellow-350 active:bg-yellow-500 text-amber-950 rounded-lg text-xs font-bold transition-colors shadow-md"
                      >
                        Nghe Khen
                      </button>
                    </div>
        ...
        <div className="text-xs text-slate-500 font-medium px-2 flex items-center gap-1">
          💡 <span>Xem nét viết mẫu ở cột bên phải, sau đó dùng chuột hoặc ngón tay để vẽ phấn tự do lên bảng nhé!</span>
        </div>
      </div>
